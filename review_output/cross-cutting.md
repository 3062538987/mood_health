...
| # | 位置 | 问题 | 严重度 |
|---|---|---|---|
| S1 | `mood_health_ai_service/.env:1` | **真实 DeepSeek 密钥泄露**：`AI_API_KEY=[REDACTED]`。该文件被 `.gitignore` 忽略（未入库，`git ls-files` 确认 NOT TRACKED），但明文存在于工作区磁盘，等同泄露，应立即吊销轮换。 | P0 |
...
|---|---|---|---|---|
| 1 | P0 | secret:`mood_health_ai_service/.env:1` | 真实 DeepSeek 密钥 `[REDACTED]` 明文落盘 | 立即吊销并轮换；迁移至密钥管理（Vault/云 Secret）；禁止在 `.env` 存真实密钥 |
