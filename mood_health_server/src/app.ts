...
import { createAiRateLimiter } from './utils/ai/rateLimiter'
...
const validateEnv = () => {
  ...
  // R8 修复：启用 AI 时必须配置内部服务令牌，否则 Node↔FastAPI 的 HMAC 鉴权会静默失败。
  // （token 由 start-project.ps1 注入两端；此处确保本地/测试配置不会在启用 AI 时漏配。）
  const aiEnabled = /^(true|1|yes)$/i.test(process.env.AI_ENABLED || '')
  if (aiEnabled && !process.env.AI_SERVICE_INTERNAL_TOKEN?.trim()) {
    const message =
      '服务启动失败：已启用 AI（AI_ENABLED=true），但缺少 AI_SERVICE_INTERNAL_TOKEN，Node 与 AI 服务将无法完成内部鉴权'
    logger.error(message)
    throw new Error(message)
  }
}
...
export const createApp = (dependencies: AppDependencies = {}) => {
  ...
  const requestLogFormat: morgan.FormatFn = (tokens, req, res) => {
...
app.use('/api/ai', createAiRateLimiter())
...
app.use('/api/mood-analyses', createAiRateLimiter())
app.use('/api', moodAnalysisRoutes)
app.use('/api/counseling', createAiRateLimiter())
