...
export const generateAndSaveAiReply = async (postId: number, content: string): Promise<void> => {
  try {
    ...
    logger.error('AI 回复生成失败', { postId, error: (error as Error)?.message })
  }
}
...
export const generateGentleReply = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!content || !content.trim()) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json(apiFailure(400, '内容不能为空'))
    }

    if (content.length > 1000) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json(apiFailure(400, '内容长度不能超过1000字'))
    }

    // 内容安全检测
    const filterResult = filterContent(content)
    if (!filterResult.isSafe && filterResult.severity === 'high') {
      return res.status(HTTP_STATUS.BAD_REQUEST).json(apiFailure(400, '内容包含不当信息，请修改后重试'))
    }
    ...
    logger.error('树洞回复生成失败', { error: (error as Error)?.message })
...
export const getAiReplyHandler = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (isNaN(id)) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json(apiFailure(400, '无效的帖子ID'))
    }
    ...
    logger.error('获取 AI 回复失败', { error: (error as Error)?.message })
    res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(apiFailure(500, '服务器内部错误'))
  }
}
...
export const generateAiReplyHandler = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!post) {
      return res.status(HTTP_STATUS.NOT_FOUND).json(apiFailure(404, '帖子不存在'))
    }
    ...
    logger.error('生成 AI 回复失败', { error: (error as Error)?.message })
    res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(apiFailure(500, (error as Error)?.message || 'AI 服务暂时不可用'))
  }
}