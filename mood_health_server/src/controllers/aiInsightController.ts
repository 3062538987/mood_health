...
export const generateInsight = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!insightData || !period) {
      return res.status(HTTP_STATUS.BAD_REQUEST).json(apiFailure(400, '缺少情绪数据或时间范围'))
    }
    ...
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(apiFailure(500, 'AI 分析暂时不可用，请稍后重试'))
  }
}