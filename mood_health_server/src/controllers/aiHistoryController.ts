...
export interface AiHistoryControllerDeps {
  repo?: ReturnType<typeof createAiHistoryRepository>
}

export const createAiHistoryController = (deps: AiHistoryControllerDeps = {}) => {
  let aiHistoryRepo = deps.repo
  const getRepo = () => {
    if (!aiHistoryRepo) {
      aiHistoryRepo = createAiHistoryRepository()
    }
    return aiHistoryRepo
  }
   ...
   * POST /api/ai/history
   */
  const saveHistory = async (req: AuthRequest, res: Response) => {
    try {
      if (!req.user) {
      ...
      if (!analysis_type || !analysis_content) {
        return res.status(HTTP_STATUS.BAD_REQUEST).json(apiFailure(API_ERROR_CODES.BAD_REQUEST, '缺少必要参数'))
      }

      const id = await getRepo().saveHistory({
      ...
      res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(apiFailure(500, '保存失败'))
    }
  }
   ...
   */
  const listHistory = async (req: AuthRequest, res: Response) => {
    try {
      if (!req.user) {
        return res.status(HTTP_STATUS.UNAUTHORIZED).json(apiFailure(401, '未登录'))
      }
      ...
      const result = await getRepo().listHistory({ userId: req.user.userId, page, pageSize })
      ...
      res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(apiFailure(500, '获取历史记录失败'))
    }
  }
   ...
   */
  const getHistoryDetail = async (req: AuthRequest, res: Response) => {
    try {
      if (!req.user) {
        return res.status(HTTP_STATUS.UNAUTHORIZED).json(apiFailure(401, '未登录'))
      }
      ...
      if (isNaN(id)) {
        return res.status(HTTP_STATUS.BAD_REQUEST).json(apiFailure(API_ERROR_CODES.BAD_REQUEST, '无效的 ID'))
      }

      const record = await getRepo().getHistoryDetail(id)
      if (!record) {
        return res.status(HTTP_STATUS.NOT_FOUND).json(apiFailure(API_ERROR_CODES.NOT_FOUND, '记录不存在'))
      }
      ...
      res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(apiFailure(500, '获取详情失败'))
    }
  }
...
const defaultController = createAiHistoryController()
export const saveHistory = defaultController.saveHistory
export const listHistory = defaultController.listHistory
export const getHistoryDetail = defaultController.getHistoryDetail