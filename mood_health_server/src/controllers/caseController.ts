...
import type { InterventionType } from '../repositories/caseRepository'
...
export const createCase = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(HTTP_STATUS.CREATED).json(apiSuccess(c, '个案创建成功'))
  } catch (error) {
    next(error)
  }
}
...
export const addIntervention = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const intervention = await caseService.addIntervention({
      ...
      interventionType: interventionType as InterventionType,
      content,
      referralTarget,
      referralReason,
      closureSummary,
    })
    res.status(HTTP_STATUS.CREATED).json(apiSuccess(intervention, '干预记录创建成功'))
  } catch (error) {
    next(error)
  }
}
...
export const getCaseDetail = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (!detail) {
      return res.status(HTTP_STATUS.NOT_FOUND).json(apiFailure(404, '个案不存在'))
    }
