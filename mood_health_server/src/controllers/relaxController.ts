...
export const saveRelaxRecordHandler = async (
  ...
  try {
    ...
    if (isNaN(start) || isNaN(end) || end <= start) {
      return res.status(400).json(apiFailure(400, "时间参数无效"));
    }
    ...
    res.status(500).json(apiFailure(500, "保存放松记录失败，请稍后重试"));
  }
};
...
export const getRelaxRecordsHandler = async (
  ...
  try {
    ...
    res.status(500).json(apiFailure(500, "获取放松记录失败，请稍后重试"));
  }
};
...
export const getRelaxStatisticsHandler = async (
  ...
  try {
    ...
    res.status(500).json(apiFailure(500, "获取放松统计失败，请稍后重试"));
  }
};
...
export const getRelaxRecordDetailHandler = async (
  ...
  try {
    ...
    res.status(500).json(apiFailure(500, "获取放松记录详情失败，请稍后重试"));
  }
};