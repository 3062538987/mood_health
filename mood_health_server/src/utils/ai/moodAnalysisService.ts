...
// AI 返回的 JSON 解析结果（情绪分析）
interface ParsedMoodAnalysis {
  risk_level?: string;
  mood_score?: number;
  mood?: string;
  analysis?: string;
...
5. 保持温和、共情的语气`
  ...
  async analyzeMood(request: MoodAnalysisRequest): Promise<MoodAnalysisResult> {
    ...
    if (request.historicalData && request.historicalData.length > 0) {
      ...
      historyText = '\n近期情绪记录：' + recent.map((r: MoodHistoryItem) => {
    ...
    try {
      ...
      // A3-04: 禁止默认值伪装完整结果
      if (typeof parsed.risk_level !== 'string' || !['low', 'medium', 'high'].includes(parsed.risk_level as string)) {
        source = 'rule'
        isFallback = true
  ...
  async predictMoodTrend(
    ...
    const historySummary = request.historicalData
      .map((r: MoodHistoryItem) => `${r.date || ''}: 强度${r.intensity || r.score || ''}/10`)
      .join('；');
