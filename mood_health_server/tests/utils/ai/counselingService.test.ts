...
describe('心理咨询控制器测试', () => {
  ...
  beforeEach(() => {
    mockedCallChatCompletion.mockReset()
    jsonMock = jest.fn().mockReturnThis()
    statusMock = jest.fn().mockReturnValue({ json: jsonMock })
    res = { status: statusMock, json: jsonMock }
    req = { user: { userId: 1, username: 'test_student', role: 'student' } as any }
  ...
  it('应该拒绝过长的消息', async () => {
    req = { ...req, body: { message: 'a'.repeat(1001) } }
    await counselingHandler(req as AuthRequest, res as Response)
    expect(statusMock).toHaveBeenCalledWith(400)
    expect(jsonMock).toHaveBeenCalledWith(
  ...
  it('应该成功生成心理咨询响应', async () => {
    ...
    req = { ...req, body: { message: '我最近感到很焦虑，不知道该怎么办' } }
  ...
  it('应该检测到风险内容并设置相应的风险级别', async () => {
    ...
    req = { ...req, body: { message: '我想自杀，觉得活不下去了' } }
    ...
    expect(jsonMock).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        data: expect.objectContaining({
          ...
          provider: null,
          model: null,
          response: expect.stringContaining('未生成真实 AI 回复'),
        }),
      })
    )
  })
