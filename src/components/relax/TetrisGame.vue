<template>
  <div class="tetris-game">
    ...
    <!-- 游戏界面 -->
    <div v-if="isGameStarted && !gameOver" class="game-container">
      <div class="game-area">
        <!-- 游戏网格 -->
        <div class="game-grid">
          <div
...
// 在网格上绘制方块
const drawPiece = (piece: Piece, xOffset: number, yOffset: number, grid: number[]) => {
  const newGrid = [...grid]
...
const movePiece = (dx: number, dy: number) => {
  if (checkCollision(currentPiece.value, currentPiece.value.x + dx, currentPiece.value.y + dy)) {
    if (dy > 0) {
      // 方块落地，锁定到网格
      lockPiece()
      clearLines()
  ...
  for (let y = 0; y < currentPiece.value.shape.length; y++) {
    for (let x = 0; x < currentPiece.value.shape[y].length; x++) {
      if (currentPiece.value.shape[y][x]) {
        const gridIndex = (y + currentPiece.value.y) * GRID_WIDTH + (x + currentPiece.value.x)
...
const clearLines = () => {
  ...
  if (linesToClear.length > 0) {
    ...
    score.value += lineScores[linesToClear.length as keyof typeof lineScores] || 0
...
const updateNextPieceGrid = () => {
  nextPieceGrid.value = Array(NEXT_PIECE_GRID_SIZE * NEXT_PIECE_GRID_SIZE).fill(0)

  const offsetX = Math.floor((NEXT_PIECE_GRID_SIZE - nextPiece.value.shape[0].length) / 2)
  const offsetY = Math.floor((NEXT_PIECE_GRID_SIZE - nextPiece.value.shape.length) / 2)
