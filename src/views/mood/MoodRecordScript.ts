...
import { saveAdviceSafe } from '@/api/advice'
...
const availableTags = ['焦虑', '开心', '疲惫', '平静', '愤怒', '沮丧', '兴奋', '紧张']
...
const suggestTriggers = () => {
  ...
  suggestions.value = commonTriggers
    .filter((t) => t.toLowerCase().includes(input) && !selectedTriggers.value.includes(t))
    .slice(0, 5)
}
...
const handleGetAdvice = async () => {
  ...
  try {
    ...
    try {
      const saveResult = await saveAdviceSafe({
        analysis: res.analysis,
        suggestions: res.suggestions,
      })

      if (!saveResult.ok) {
        ElMessage.warning(saveResult.message || '建议生成成功，但保存失败')
      }
    } catch (saveError) {
