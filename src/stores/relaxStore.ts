...
/**
  ...
  async function saveRecord(record: Omit<RelaxRecord, "id" | "userId">) {
    ...
    try {
      ...
      if (userId) {
        // 已登录，直接保存到后端
        const savedRecord = await relaxAPI.saveRecord({
          ...record,
          userId,
        });
  ...
  async function fetchRecords(params?: {
    ...
    try {
      ...
      if (userId) {
        const response = await relaxAPI.getRecords(params);
        records.value = response.records;
        return response;
      } else {
        // 未登录，从localStorage获取暂存记录
        const storedRecords =
          storageUtil.getItem<RelaxRecord[]>("pendingRelaxRecords") || [];
  ...
  async function fetchStatistics(params?: {
    ...
    try {
      ...
      if (userId) {
        const data = await relaxAPI.getStatistics(params);
  ...
  function calculateLocalStatistics(params?: {
    ...
    if (params?.startDate) {
      filteredRecords = filteredRecords.filter(
        (record) => record.startTime >= params.startDate!,
      );
    }

    if (params?.endDate) {
      filteredRecords = filteredRecords.filter(
        (record) => record.startTime <= params.endDate!,
      );
    }
    ...
    const activityBreakdown = Object.entries(activityCounts).map(
      ([type, count]) => {
        const typeRecords = filteredRecords.filter(
          (r) => r.activityType === (type as any),
        );
