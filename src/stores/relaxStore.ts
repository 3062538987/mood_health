    ...
    try {
      if (userStore.isLoggedIn) {
        ...
        const savedResult = await relaxAPI.saveRecordSafe({
          ...
          clientTimestamp: Date.now() as any,
        })
        if (!savedResult.ok) {
        ...
        const pendingRecord = { ...record, id: `local-${Date.now()}`, userId: 'anonymous' }
        records.value.unshift(pendingRecord as any)
      ...
      records.value.unshift(fallbackRecord as any)
    ...
    for (const item of queue) {
      ...
      try {
        await relaxAPI.saveRecord({
          ...
          clientTimestamp: item.clientTimestamp as any,
        })
      } catch {
        // 同步失败，增加重试计数并保留
        remaining.push({
