...
// This test asserts a production performance envelope (load < 1500ms, no long tasks,
// no repeated requests). It is only meaningful against a *production build* served by a
// static server. Under the vite dev-server E2E (unbundled modules + HMR) those assertions
// cannot hold, so it is skipped by default. Opt in with E2E_PERF=1 when running against a build.
test.skip(process.env.E2E_PERF !== '1', 'requires a production build + realistic environment; enable with E2E_PERF=1')
