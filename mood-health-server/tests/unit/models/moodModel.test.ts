...
describe('moodModel', () => {
  ...
  describe('createMood', () => {
    ...
    it('should create mood with intensity at boundaries', async () => {
      const moodIdMin = await createMood(testUserId, '悲伤', 1, '', '', '', testDate)
      expect(moodIdMin).toBeGreaterThan(0)

      const moodIdMax = await createMood(testUserId, '快乐', 10, '', '', '', testDate)
  ...
  describe('findMoodById', () => {
    ...
    beforeEach(async () => {
      createdMoodId = await createMood(testUserId, '快乐', 7, '测试记录', '运动', '', testDate)
    })
  ...
  describe('updateMood', () => {
    ...
    beforeEach(async () => {
      moodId = await createMood(testUserId, '悲伤', 3, '原始记录', '', '', testDate)
    })
  ...
  describe('getMoodAnalysis', () => {
    ...
    it('should calculate emotion ratios correctly', async () => {
      ...
      const total = analysis.positiveRatio + analysis.negativeRatio + analysis.neutralRatio
    ...
    it('should return trend direction as valid value', async () => {
      ...
      expect(['improving', 'declining', 'stable']).toContain(analysis.trendDirection)
    })
  })
