...
export const recordMood = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (emotions && Array.isArray(emotions) && emotions.length > 0) {
      for (const emotion of emotions) {
        if (!emotion.emotionTypeId || emotion.intensity === undefined) {
          return res
            .status(400)
            .json({ code: 400, message: "情绪数据格式错误" });
        }
        if (emotion.intensity < 1 || emotion.intensity > 10) {
          return res
            .status(400)
            .json({ code: 400, message: "强度必须在1-10之间" });
        }
      }
    ...
    if (!moodType || !moodRatio) {
      return res
        .status(400)
        .json({ code: 400, message: "情绪类型和强度为必填" });
    }
...
export const getWeeklyReportHandler = async (
  req: AuthRequest,
  res: Response,
) => {
  try {
...
export const getMoodAnalysisHandler = async (
  req: AuthRequest,
  res: Response,
) => {
  try {
...
export const getAdviceHistoryHandler = async (
  req: AuthRequest,
  res: Response,
) => {
  try {
