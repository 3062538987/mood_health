import { connectDB, pool, isSqliteClient } from '../config/database'
import sql from 'mssql'
import { sqliteAll } from '../config/sqlite'
...
const testQuestionnaireHistory = async () => {
  try {
    ...
    const result = await pool.request().input('userId', sql.Int, testUserId).query(historyQuery)
