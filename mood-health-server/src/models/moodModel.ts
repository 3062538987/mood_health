...
export const createMoodWithRelations = async (
  ...
  try {
    ...
    const moodTypeNames = await getEmotionTypeNames(emotions.map((e) => e.emotionTypeId))
    ...
    for (const tagId of tagIds) {
      await transaction.request().input('moodId', sql.Int, moodId).input('tagId', sql.Int, tagId)
        .query(`
...
export const getMoodsWithRelations = async (
  ...
  const emotionPlaceholders = moodIds.map((_, index) => `@moodId${index}`).join(',')
  ...
  const tagPlaceholders = moodIds.map((_, index) => `@moodId${index}`).join(',')
  moodIds.forEach((id, index) => {
...
export const getMoodAnalysis = async (
  ...
  const dominantEmotion = sortedEmotions.length > 0 ? sortedEmotions[0][0] : null
  ...
  for (const date of sortedDates) {
    ...
    if (record.intensity <= LOW_INTENSITY_THRESHOLD && record.negativeCount > 0) {
      consecutiveLowDays++
    } else {
      break
    }
  ...
  if (sortedByDateAsc.length >= 3) {
    const firstHalf = sortedByDateAsc.slice(0, Math.floor(sortedByDateAsc.length / 2))
    const secondHalf = sortedByDateAsc.slice(Math.floor(sortedByDateAsc.length / 2))

    const firstHalfAvg =
      firstHalf.reduce((sum, date) => sum + dateRecords[date].intensity, 0) / firstHalf.length
    const secondHalfAvg =
      secondHalf.reduce((sum, date) => sum + dateRecords[date].intensity, 0) / secondHalf.length
...
const generateRecommendations = (
  ...
  if (consecutiveLowDays >= CONSECUTIVE_ALERT_THRESHOLD) {
    recommendations.push(`你已连续${consecutiveLowDays}天情绪低落，建议进行户外运动或与朋友倾诉`)
  ...
  for (const [trigger] of topTriggers) {
    recommendations.push(`"${trigger}"常引发负面情绪，建议思考如何改善这种情况`)
  }
...
export const findMoodWithRelationsById = async (
  ...
  const emotionsResult = await pool.request().input('moodId', sql.Int, id).query(`
      SELECT me.mood_id, me.emotion_type_id, me.intensity, et.name as emotion_name, et.icon as emotion_icon
      FROM mood_emotions me
      JOIN emotion_types et ON me.emotion_type_id = et.id
...
export const updateMoodWithRelations = async (
  ...
  try {
    ...
    for (const tagId of tagIds) {
      await transaction.request().input('moodId', sql.Int, id).input('tagId', sql.Int, tagId)
        .query(`
          INSERT INTO mood_tags (mood_id, tag_id)
          VALUES (@moodId, @tagId)
        `)
...
export const getMoodTrend = async (userId: number, range: string) => {
  ...
  const datasets = moodTypes.map((type: string) => {
    const data = dates.map((date: string) => {
      const record = records.find(
        (r: { date: string; mood_type: string }) => r.date === date && r.mood_type === type
      )
  ...
  if (records.length === 0) {
    ...
    const avgIntensity =
      records.reduce((sum: number, r: { avg_intensity: number }) => sum + r.avg_intensity, 0) /
      records.length
    if (avgIntensity >= 4) {
