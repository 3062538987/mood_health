...
Start-Process powershell -ArgumentList @(
    '-NoExit',
    '-Command',
    "`$host.UI.RawUI.WindowTitle='MoodHealth - FastAPI AI'; Write-Host '=== FastAPI AI (port $AiPort) ===' -ForegroundColor Cyan; `$env:MOOD_AI_SERVICE_PORT='$AiPort'; Set-Location '$Root\mood_health_ai_service'; uvicorn app.main:app --host 0.0.0.0 --port $AiPort --reload"
)
...
Start-Process powershell -ArgumentList @(
    '-NoExit',
    '-Command',
    "`$host.UI.RawUI.WindowTitle='MoodHealth - Node Backend'; Write-Host '=== Node Backend (port $NodePort) ===' -ForegroundColor Cyan; `$env:PORT='$NodePort'; `$env:FASTAPI_BASE_URL='http://127.0.0.1:${AiPort}'; Set-Location '$Root\mood_health_server'; npm run dev"
)
